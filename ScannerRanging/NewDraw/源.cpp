...
#include "../VirtualPostClient/CoordTrans.h"
...
int main()
{
	...
	auto& device = LidarDevice::DeviceList.begin()->second;
	infoBox.setArg("Angle", to_tstring((int)device.rawData.angleBeg) + '~' + to_tstring((int)device.rawData.angleEnd));
	CoordTrans coord;
	...
	BeginBatchDraw();
	FlushMouseMsgBuffer();
	while (LidarDevice::OnlineDeviceNumber > 0)
	{
		if (MouseHit())
		{
		...
		if (bIfBg)
		{
			...
			solidpie(-10000, -10000, 10000, 10000, device.rawData.angleBeg * PI / 180, device.rawData.angleEnd * PI / 180);
		...
		device.LockRawData();
		coord.ProcessCoord(device.rawData);
		device.UnlockRawData();
		...
		if (bIfArea || bIfLine)
		{
			...
			if (bIfLine)
			{
				setlinestyle(PS_SOLID, 1);
				setlinecolor(GREEN);
				if (!coord.legalCoord.empty())
				{
					moveto(coord.legalCoord.front().y, -coord.legalCoord.front().h);
					for (auto& [x, y] : coord.legalCoord)
					{
						lineto(x, -y);
					}
				}
				...
				if (!coord.legalCoord.empty())
				{
					circle(coord.legalCoord.front().y, -coord.legalCoord.front().h, 2);
					for (auto& [x, y] : coord.legalCoord)
					{
						circle(x, -y, 2);
					}
				...
				for (size_t i = 0; i < RAW_DATA_SIZE; i++)
				{
					if (coord.legalρ[i] >= 0)
					{
						moveto(coord.legalρ[i] * cos(CoordTrans::index2Angle(i) / 180 * PI), -coord.legalρ[i] * sin(CoordTrans::index2Angle(i) / 180 * PI));
						break;
					}
				}
				for (size_t i = 0; i < RAW_DATA_SIZE; i++)
				{
					if (coord.legalρ[i] > 0)
					{
						int x = coord.legalρ[i] * cos(CoordTrans::index2Angle(i) / 180 * PI);
						int y = -coord.legalρ[i] * sin(CoordTrans::index2Angle(i) / 180 * PI);
						lineto(x, y);
					}
				}
...
#define TEXT_ARC(_len) \
			arc(-_len##00,-_len##00,_len##00,_len##00, coord.rawData.angleBeg * PI / 180, coord.rawData.angleEnd * PI / 180); \
			outtextxy(_len##00*cos(-coord.rawData.angleBeg * PI / 180)-10, _len##00*sin(-coord.rawData.angleBeg * PI / 180), TEXT(#_len##"m"));
...
#undef TEXT_ARC
		...
		if (bIfInfo)
		{
			...
			if (Msg.mkRButton)
			{
				...
				outtextxy((Msg.x + mousePos.x) / 2, (Msg.y + mousePos.y) / 2, (to_tstring(sqrt((Msg.x - mousePos.x) * (Msg.x - mousePos.x) + (Msg.y - mousePos.y) * (Msg.y - mousePos.y)) / scale / 100) + tstring(TEXT("m"))).c_str());
				outtextxy((Msg.x + originPos.x) / 2, (Msg.y + originPos.y) / 2, (to_tstring(sqrt((Msg.x - originPos.x) * (Msg.x - originPos.x) + (Msg.y - originPos.y) * (Msg.y - originPos.y)) / scale / 100) + tstring(TEXT("m"))).c_str());
				outtextxy(Msg.x, Msg.y + 30, (to_tstring(atan2(Msg.y - originPos.y, -Msg.x + originPos.x) * 180 / PI + 180) + tstring(TEXT("°"))).c_str());
				setaspectratio(scale, scale);
				setorigin(originPos.x, originPos.y);
			}
		}
